class SepPair1 {
    index: number;
    char: string;
    constructor(index1: number, char1: string) {
        this.index = index1;
        this.char = char1;
    }
    isSpecial(): boolean {
...
class XLexem {
    ...
    constructor(type1: XLexemType, content1: string = null) {
        this.type = type1;
...
class XParser {
    ...
    private isClosed(lexem: XLexem): boolean {
     ...
     */
    private reStackBiggerLexem(stack: XLexem[]): XLexem {
...
class XLexemFormatter {
    ...
    private formatLexem(lexem: XLexem): string {
        let result: string;
...
function highlightAllAS4X_3() {
    ...
    for (let i = 0; i < codes.length; i++) {
        const codeTag = codes[i] as HTMLPreElement;
        const preTag = codeTag.parentElement as HTMLPreElement;
